...
class TestBlock(CustomAssertions):
    ...
    def test_blocks_have_been_deleted(self):
        ...
        for important_block in IMPORTANT_BLOCKS:
            self.assertTrue(
                important_block in blocks,
                f"{important_block} should be displayed by insel -b",
            )
        for deleted_block in deleted_blocks:
            self.assertFalse(
                deleted_block in blocks, f"{deleted_block} should have been deleted."
            )
    ...
    def test_gain(self):
        ...
        self.assertEqual(
            len(insel.block("gain", *range(10), parameters=[5], outputs=10)),
            10,
            "10 inputs should be enough for GAIN",
        )
    ...
    def test_exp(self):
        ...
        self.assertEqual(
            " ".join(
                [
                    "%.2f" % x
                    for x in insel.block("exp", -3.5, -2.0, 1.4, 2.6, 4.7, outputs=5)
                ]
            ),
    ...
    def test_nop(self):
        ...
        self.assertEqual(
            " ".join(
                [
                    "%.2f" % x
                    for x in insel.block("nop", -3.5, -2.0, 1.4, 2.6, 4.7, outputs=5)
                ]
            ),
    ...
    def test_chs(self):
        ...
        self.assertEqual(
            " ".join(
                [
                    "%.2f" % x
                    for x in insel.block("chs", 3.5, 2.0, -1.4, -2.6, -4.7, outputs=5)
                ]
            ),
            "-3.50 -2.00 1.40 2.60 4.70",
        )
    ...
    def test_frac(self):
        ...
        self.assertEqual(
            " ".join(
                [
                    "%.1f" % x
                    for x in insel.block("frac", 3.5, 2.0, -1.4, -2.6, -4.7, outputs=5)
                ]
            ),
            "0.5 0.0 -0.4 -0.6 -0.7",
        )
    ...
    def test_mtm_with_incorrect_input(self):
        ...
        self.assertTrue("Block 00002: Invalid input" in str(warnings))
    ...
    def test_moonae(self):
        ...
        moon_stuttgart = insel.block(
            "MOONAE2", 2021, 2, 18, 23, 33, parameters=STUTTGART, outputs=2
        )
        self.compareLists(moon_stuttgart, [279, 13], places=0)
        moon_stuttgart = insel.block(
            "MOONAE2", 2021, 2, 23, 5, 7, 30, parameters=STUTTGART, outputs=2
        )
        self.compareLists(moon_stuttgart, [308.5, 0], places=0)
        # Tested with http://www.stjarnhimlen.se/comp/tutorial.html#9
        moon_sweden = insel.block(
            "MOONAE2", 1990, 4, 19, 2, parameters=[60, 15, 2], outputs=5
        )
        self.compareLists(
            moon_sweden,
            [101 + 46.0 / 60, -16 - 11.0 / 60, -19.9, 272.3 - 0.5, 100],
            places=0,
        )

        moon_stuttgart = insel.block(
            "MOONAE2", 2021, 5, 26, 13, 13, parameters=STUTTGART, outputs=5
        )
        self.assertTrue(moon_stuttgart[4] < 2.0, "26.05.2021 should be a full moon.")

        moon_stuttgart = insel.block(
            "MOONAE2", 2021, 6, 10, 12, 0, parameters=STUTTGART, outputs=5
        )
        self.assertTrue(moon_stuttgart[4] > 178, "10.06.2021 should be a new moon.")

        # It should work at the equator too:
        self.assertNotNaN(
            insel.block("moonae2", 2021, 11, 19, 12, parameters=[0, 0, 0])
        )

    def test_sunae(self):
        for mode in range(3):  # type: insel.Parameter
            # Tested with Stellarium
            sun_stuttgart = insel.block(
                "SUNAE2", 2021, 11, 18, 12, 0, parameters=[mode] + STUTTGART, outputs=4
            )
    ...
    def test_sunae_in_the_tropics(self):
        ...
        sun_azimuth = insel.block("SUNAE2", 2021, 6, 21, 6, 0, parameters=[0, 20, 0, 0])
    ...
    def test_gregorian_date(self):
        self.compareLists(insel.block("gregor", 2_415_021, outputs=3), [1900, 1, 1])
        self.compareLists(insel.block("gregor", 2_451_545, outputs=3), [2000, 1, 1])
        self.compareLists(insel.block("gregor", 2_459_694, outputs=3), [2022, 4, 24])
        self.compareLists(insel.block("gregor", 2_488_069, outputs=3), [2099, 12, 31])

    def test_weighted_average(self):
        self.assertAlmostEqual(65, insel.block("avew", 50, 80, parameters=[1, 1]))
        self.assertAlmostEqual(62, insel.block("avew", 80, 50, parameters=[4, 6]))
        self.assertAlmostEqual(10, insel.block("avew", 0, 10, parameters=[0, 1]))
        self.assertAlmostEqual(10, insel.block("avew", 10, 0, parameters=[1, 0]))
        self.assertRaisesRegex(
            InselError,
            "Invalid parameter",
            insel.block,
    ...
    def test_screen(self):
        ...
        self.assertRaisesRegex(InselError, "Invalid output", insel.block, "screen")

    def test_screen1g(self):
        # SCREEN1G has no output
        self.assertRaisesRegex(InselError, "Invalid output", insel.block, "screen1g")
...
class TestGenericExpression(CustomAssertions):
    ...
    def test_wrong_formulas(self):
        self.assertRaisesRegex(
            InselError, r" \^ First error is here", self.expr, ") 2 + 3"
        )
        self.assertRaisesRegex(
            InselError, r" __\^ First error is here", self.expr, "x + "
        )
        self.assertRaisesRegex(
            InselError, r" ________\^ First error is here", self.expr, "sin(1 * )"
        )
        self.assertRaisesRegex(
            InselError, r" ____\^ First error is here", self.expr, "1 + a + b"
        )

    def test_missing_x(self):
        self.assertRaisesRegex(InselError, "Unknown variable 'x'", self.expr, "x + 3")

    def test_missing_y(self):
        self.assertRaisesRegex(
            InselError, "Unknown variable 'y'", self.expr, "x + y", 1
        )

    def test_missing_z(self):
        self.assertRaisesRegex(
            InselError, "Unknown variable 'z'", self.expr, "x + y + z", 1, 2
        )
