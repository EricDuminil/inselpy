...
from .constants import IS_WINDOWS, SCRIPT_DIR
...
class TestBasicTemplates(CustomAssertions):
    def test_a_times_b(self):
        ...
        self.assertAlmostEqual(
            insel.template(SCRIPT_DIR / "templates" / "a_times_b.insel", a=4, b=5),
            20,
            places=6,
        )
    ...
    def test_array_parameter(self):
        self.assertRaisesRegex(
            AttributeError, "UndefinedValue for 'x'", insel.template, "array_parameters"
        )
        self.assertRaisesRegex(
            AttributeError,
        ...
        self.assertEqual(6, insel.template("array_parameters", x={0: 1, 1: 3, 2: 2}))
...
class TestTemplatesWithConstants(CustomAssertions):
    ...
    def test_setting_string_constants_in_vseit(self):
        self.assertListEqual(
            [72, 109, 173, 227, 257, 279, 301, 270, 200, 132, 83, 74],
            insel.template("constants/string_constant"),
        )
        self.assertListEqual(
            [326, 300, 250, 168, 122, 101, 107, 141, 198, 244, 302, 335],
        ...
        self.assertRaisesRegex(
            ...
            location_name="NotACity",
        )

    def test_ignore_units_in_defcon(self):
        self.assertAlmostEqual(
            123 + 123.456 + 48.7 + 456.789,
            insel.template("constants/ignore_units_in_defcon.vseit"),
        )
        self.assertAlmostEqual(
            123 + 123.456 + 48.7 + 12,
            insel.template("constants/ignore_units_in_defcon.vseit", Energy=12),
        )
    ...
    def test_placeholder_over_constant(self):
        self.assertEqual(
            12,
            insel.template(
                "constants/placeholder_too", placeholder_x=5, placeholder_y=7
            ),
        )
        self.assertEqual(
            12,
            insel.template(
                "constants/placeholder_too",
                placeholder_x=5,
                placeholder_y=7,
                constant_x=0,
                constant_y=0,
...
class TestTemplates(CustomAssertions):
    ...
    def test_mtm_averages(self):
        ...
        places = [
            ...
            "Naesgard",
            "Nurnberg",
            "Planes de Montecrist",
        ]
        for place in places:
            self.assertTrue(
                insel.template("weather/check_temperatures", location=place),
    ...
    def test_updated_coordinates(self):
        v1_results = insel.template(
            "photovoltaic/nurnberg_v1",
            latitude=49.5,
            old_longitude=-11.08,
            old_timezone=23,
        )
    ...
    def test_max_xy(self):
        ...
        self.assertEqual(insel.template("stats/max_xy_c"), [[2020, 31], [2021, 31]])
    ...
    def test_min_xy(self):
        ...
        self.assertEqual(insel.template("stats/min_xy_c"), [[2020, 29], [2021, 28]])
    ...
    def test_sunpower_uoc(self):
        ...
        self.assertAlmostEqual(
            insel.template("photovoltaic/u_oc", pv_id="003305"), 64.2, places=2
        )
        # TODO: More research is needed :)
        self.skipTest("Not sure about this calculation.")
        temp = 70
        self.assertAlmostEqual(
            insel.template("photovoltaic/u_oc", pv_id="003305", temperature=temp),
            64.2 + (temp - 25) * (-0.1766),
            places=2,
        )
    ...
    def test_sunpower_mpp(self):
        ...
        self.assertAlmostEqual(
            insel.template("photovoltaic/mpp", pv_id="003305"), 305, places=0
        )
        temp = 70
        ...
        self.assertAlmostEqual(
            insel.template("photovoltaic/mpp", pv_id="003305", temperature=temp),
    ...
    def test_write_block(self):
        ...
        self._run_write_block(basename="Ñüößç&txt.täxt€", header="#ßeäöütµ§%&²³@°")
    ...
    def test_write_block_relative_file_with_long_filename(self):
        ...
        insel.template(
            "io/write_params", dat_file=dat_file.name, run_in_templates_folder=False
        )
    ...
    def test_read_missing_file(self):
        self.assertRaisesRegex(
            InselError,
            "(?m)^F05029 Block 00001: Cannot open file: .*not_here$",
            insel.template,
            "io/not_here",
        )
